...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  if (authForm) {
    authForm.addEventListener("submit", async (e) => {
      ...
      try {
        validPass = await window.dataManager.getAdminPassword();
      } catch (err) {
        console.warn("Erreur lecture mot de passe depuis Supabase :", err);
      }
  ...
  async function loadAllData() {
    if (!window.dataManager.isSupabaseConnected) {
      showToast("⚠️ Supabase n'est pas encore configuré. Renseignez vos accès ci-dessous.", "info");
      switchTab("settings");
      return;
    }
  ...
  if (productForm) {
    productForm.addEventListener("submit", async (e) => {
      ...
      try {
        ...
        closeProdModal();
        await refreshAll();
        showToast(id ? "Article modifié avec succès dans Supabase !" : "Nouvel article ajouté avec succès dans Supabase !", "success");
      } catch (err) {
  ...
  if (categoryForm) {
    categoryForm.addEventListener("submit", async (e) => {
      ...
      try {
        ...
        closeCatModal();
        await refreshAll();
        showToast(id ? "Catégorie modifiée avec succès dans Supabase !" : "Catégorie créée avec succès dans Supabase !", "success");
      } catch (err) {
  ...
  // 8. VENTES MANUELLES & HISTORIQUE COMPLET DES ENREGISTREMENTS
  const posHistoryTableBody = document.getElementById("pos-history-table-body");
  const posHistorySearch = document.getElementById("pos-history-search");
  const posHistoryBadge = document.getElementById("pos-history-badge");
  const posKpiTotalRevenue = document.getElementById("pos-kpi-total-revenue");
  const posKpiTotalCount = document.getElementById("pos-kpi-total-count");
  const posPaymentMethod = document.getElementById("pos-payment-method");

  function renderPOSHistory() {
    if (!posHistoryTableBody) return;
    posHistoryTableBody.innerHTML = "";

    // Filtrer uniquement les ventes directes émanant de la caisse / magasin
    const manualSales = sales.filter(s => s.sale_type === "magasin_direct" || !s.sale_type);

    // Calculs totaux
    const totalManualRev = manualSales.reduce((acc, s) => acc + (parseFloat(s.total_amount) || 0), 0);
    const totalManualCount = manualSales.reduce((acc, s) => acc + (parseInt(s.quantity, 10) || 0), 0);

    if (posKpiTotalRevenue) posKpiTotalRevenue.textContent = formatPrice(totalManualRev);
    if (posKpiTotalCount) posKpiTotalCount.textContent = `${manualSales.length} enregistrement(s) (${totalManualCount} articles)`;
    if (posHistoryBadge) posHistoryBadge.innerHTML = `<span class="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse"></span> ${manualSales.length} enregistrement(s)`;

    const query = (posHistorySearch ? posHistorySearch.value : "").trim().toLowerCase();
    const filtered = manualSales.filter(s => {
      const title = (s.product_title || "").toLowerCase();
      const method = (s.payment_method || "").toLowerCase();
      const date = formatDate(s.created_at).toLowerCase();
      return !query || title.includes(query) || method.includes(query) || date.includes(query);
    });
      ...
      posHistoryTableBody.innerHTML = `
        <tr>
          <td colspan="7" class="px-6 py-8 text-center text-slate-400">
            ...
            Aucun enregistrement de vente manuelle trouvé.
          </td>
        </tr>
      `;
      if (window.lucide) window.lucide.createIcons();
    ...
    filtered.forEach(s => {
      const tr = document.createElement("tr");
      tr.className = "border-b border-slate-100 hover:bg-amber-50/20 transition text-sm";
      tr.innerHTML = `
        <td class="px-5 py-3.5">
          <div class="font-bold text-slate-900">${formatDate(s.created_at)}</div>
          <div class="text-[10px] text-slate-400 font-mono">ID: ${(s.id || '').substring(0, 8)}</div>
        </td>
        <td class="px-5 py-3.5">
          <div class="font-bold text-slate-900">${s.product_title || 'Article direct'}</div>
          <div class="text-[11px] text-slate-500">Vente directe comptoir</div>
        </td>
        <td class="px-5 py-3.5 text-center font-extrabold text-slate-800">
          <span class="px-2.5 py-1 rounded-full text-xs bg-slate-100 text-slate-700">${s.quantity}x</span>
        </td>
        <td class="px-5 py-3.5 font-semibold text-slate-600">
          ${formatPrice(s.unit_price)}
        </td>
        <td class="px-5 py-3.5 font-black text-amber-700">
        ...
        <td class="px-5 py-3.5 text-right">
          <button class="btn-cancel-sale p-1.5 rounded-lg bg-rose-50 text-rose-600 hover:bg-rose-100 transition" data-id="${s.id}" title="Annuler cette vente et recréditer le stock">
            <i data-lucide="rotate-ccw" class="w-4 h-4"></i>
          </button>
        </td>
      `;

      tr.querySelector(".btn-cancel-sale").addEventListener("click", async () => {
        if (confirm(`Confirmez-vous l'annulation de cette vente de "${s.product_title}" (${s.quantity}x) ? Le stock sera automatiquement recrédité dans Supabase.`)) {
          try {
            await window.dataManager.deleteSale(s.id);
            await refreshAll();
            showToast("Vente annulée et stock recrédité avec succès !", "success");
          } catch (err) {
            showToast(err.message, "error");
          }
        }
      ...
      posHistoryTableBody.appendChild(tr);
    });
  ...
  if (posHistorySearch) {
    posHistorySearch.addEventListener("input", renderPOSHistory);
  }
  ...
  if (posForm) {
    posForm.addEventListener("submit", async (e) => {
      ...
      const paymentMethod = posPaymentMethod ? posPaymentMethod.value : "Espèces";

      try {
        await window.dataManager.recordDirectSale(prodId, qty, customPrice, paymentMethod);
        posForm.reset();
        updatePosTotal();
        await refreshAll();
        showToast("✅ Vente en caisse enregistrée avec succès dans Supabase !", "success");
      } catch (err) {
        console.error("Erreur vente en caisse :", err);
        showToast(err.message, "error");
      }
    });
  }
  ...
  if (sbUrlInput) sbUrlInput.value = APP_CONFIG.SUPABASE_URL || localStorage.getItem(APP_CONFIG.STORAGE_KEYS.SUPABASE_URL) || "";
  if (sbKeyInput) sbKeyInput.value = APP_CONFIG.SUPABASE_ANON_KEY || localStorage.getItem(APP_CONFIG.STORAGE_KEYS.SUPABASE_KEY) || "";

  function updateSupabaseStatusUI() {
    if (!sbStatusBadge) return;
    if (window.dataManager.isSupabaseConnected) {
      sbStatusBadge.className = "hidden sm:flex px-3 py-1 rounded-full text-xs font-bold bg-emerald-100 text-emerald-800 items-center gap-1.5";
      sbStatusBadge.innerHTML = `<span class="w-2 h-2 rounded-full bg-emerald-500"></span> Supabase Connecté`;
    } else {
      sbStatusBadge.className = "hidden sm:flex px-3 py-1 rounded-full text-xs font-bold bg-rose-100 text-rose-800 items-center gap-1.5";
      sbStatusBadge.innerHTML = `<span class="w-2 h-2 rounded-full bg-rose-500 animate-pulse"></span> Supabase Non Connecté`;
    }
  }
  ...
  if (adminPassForm) {
    adminPassForm.addEventListener("submit", async (e) => {
      ...
      try {
        await window.dataManager.setAdminPassword(newPass);
        newPassInput.value = "";
        showToast("✅ Nouveau mot de passe enregistré dans Supabase (synchronisé en ligne et en local) !", "success");
      } catch (err) {
