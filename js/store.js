  ...
  async function loadStoreData() {
    try {
      if (!window.dataManager.isSupabaseConnected) {
        if (productsGrid) {
          productsGrid.innerHTML = `
            <div class="col-span-full bg-amber-50/80 border border-amber-200 p-10 rounded-3xl text-center shadow-xs">
              <div class="w-14 h-14 rounded-2xl bg-amber-100 text-amber-800 flex items-center justify-center mx-auto mb-4">
                <i data-lucide="database" class="w-7 h-7"></i>
              </div>
              <h3 class="text-lg font-black text-slate-900 mb-1 font-serif">Base de données Supabase en attente de connexion</h3>
              <p class="text-xs text-slate-600 max-w-md mx-auto mb-4 leading-relaxed">
                Pour charger votre vrai catalogue, renseignez votre URL et Clé Anon Supabase dans <code>js/config.js</code> ou dans l'onglet réglages de l'administration.
              </p>
              <a href="admin.html" class="inline-flex items-center gap-2 px-4 py-2.5 bg-amber-600 hover:bg-amber-700 text-white rounded-xl font-bold text-xs shadow-sm transition">
                <i data-lucide="settings" class="w-4 h-4"></i> Ouvrir les Réglages Supabase
              </a>
            </div>
          `;
      ...
      [allCategories, allProducts] = await Promise.all([
        ...
        window.dataManager.getProducts(true) // Seulement les produits actifs de Supabase
      ]);
      ...
      showToast(err.message || "Erreur de connexion à Supabase", "error");
